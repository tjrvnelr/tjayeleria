...
const experiences = [
  {
    ...
    period: '2022 – Present',
    description: 'Led the frontend architecture for a SaaS platform serving 50K+ users. Reduced bundle size by 45% and improved core web vitals to top 10% globally.',
  ...
  },
  {
    ...
    location: 'Remote',
    period: '2020 – 2022',
    description: 'Built and maintained 3 production applications from the ground up. Collaborated with design, product, and data teams to ship high-impact features monthly.',
  ...
  },
  {
    ...
    period: '2019 – 2020',
    description: 'Developed responsive web interfaces and internal tooling that improved operational efficiency by 30%. Mentored junior developers and established code review practices.',
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 md:w-1/2 pb-8 ${isEven ? '' : 'md:text-right'}`}>
        <div className={`p-6 rounded-2xl bg-white border border-[#112D4E]/8 shadow-sm hover:shadow-lg hover:border-[#3F72AF]/30 transition-all duration-300 hover:-translate-y-1`}>
          <div className={`flex flex-wrap items-center gap-2 mb-1 ${isEven ? '' : 'md:flex-row-reverse'}`}>
          ...
          <div className={`flex flex-wrap gap-3 text-[#112D4E]/40 text-xs mb-3 ${isEven ? '' : 'md:flex-row-reverse'}`}>
          ...
          <p className="text-[#112D4E]/60 text-sm leading-relaxed mb-4">{item.description}</p>

          <div className={`flex flex-wrap gap-1.5 ${isEven ? '' : 'md:flex-row-reverse'}`}>
            {item.highlights.map((tag) => (
              <span key={tag} className="px-2.5 py-1 bg-[#112D4E]/5 text-[#112D4E]/55 text-xs rounded-lg font-medium">
                {tag}
              </span>
            ))}
          </div>
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-28 bg-[#112D4E] relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        >
          <span className="text-[#3F72AF] text-sm font-semibold uppercase tracking-widest">Journey</span>
        ...
        <div className="grid lg:grid-cols-2 gap-16">
          <div>
            ...
            <div className="relative">
              <div className="absolute left-6 top-6 bottom-0 w-px bg-gradient-to-b from-[#3F72AF]/40 to-transparent hidden md:block" style={{ left: '23px' }} />
...
function WorkItem({ item, index }: { item: typeof experiences[0]; index: number }) {
  const ref = useRef(null);
      ...
      <div className="flex-1 p-5 rounded-2xl bg-[#F9F7F7]/5 border border-[#F9F7F7]/8 hover:border-[#3F72AF]/30 transition-all duration-300 hover:bg-[#F9F7F7]/8">
        <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
          <div>
            ...
            <span className="text-[#3F72AF] text-sm font-medium">{item.company}</span>
          </div>
          <div className="text-right text-xs text-[#F9F7F7]/40">
            <div className="flex items-center gap-1"><FiCalendar size={10} /> {item.period}</div>
            <div className="flex items-center gap-1 mt-0.5"><FiMapPin size={10} /> {item.location}</div>
          </div>
        </div>
        <p className="text-[#F9F7F7]/50 text-sm leading-relaxed mb-3">{item.description}</p>
        <div className="flex flex-wrap gap-1.5">
          {item.highlights.map((tag) => (
            <span key={tag} className="px-2 py-0.5 bg-[#3F72AF]/15 text-[#3F72AF] text-xs rounded-md font-medium">
              {tag}
            </span>
          ))}
        </div>
...
function AchievementCard({ item, delay }: { item: typeof achievements[0]; delay: number }) {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: '-40px' });
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h4 className="font-semibold text-[#F9F7F7] text-sm leading-tight">{item.title}</h4>
            <span className="text-[#3F72AF] text-xs bg-[#3F72AF]/15 px-2 py-0.5 rounded-full shrink-0">{item.year}</span>
          </div>
          <div className="text-[#F9F7F7]/40 text-xs mt-0.5 mb-2">{item.org}</div>
          <p className="text-[#F9F7F7]/50 text-xs leading-relaxed">{item.desc}</p>
        </div>
      </div>
    </motion.div>
  );
